...
#
# Unit testing level 4: tests the full Jacobian computation and compares against JmultT.
...
if [ $# -gt 2 ]; then
#
echo "#### START FORWARD MPI TEST WITH $ncores CORES AT $(date)" | tee level4.out
#
#
echo "#### COMMAND LINE: mpirun -n $ncores $cmd "  | tee -a level4.out
#
#
eval mpirun -n $ncores $cmd | tee -a level4.out
#
else
#
echo "#### START FORWARD LOCAL TEST AT $(date)" | tee level4.out
#
#
echo "#### COMMAND LINE: $cmd "  | tee -a level4.out
#
#
eval $cmd | tee -a level4.out
#
fi
