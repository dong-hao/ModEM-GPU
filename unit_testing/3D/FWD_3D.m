...
modelFile = 'CylinderModel+QuarterSpace.rho';
...
dat = mtdata.read('CylinderModel_6per_Template.dat','list','[mV/km]/[nT]','Full_Impedance');
T = dat.periods;
nPer = length(T);
...
% load the E-fields
%SOLNSFOR = readE('CylinderModel_6per.soln',6);
%SOLNSFOR = readE('CylinderModel_1per.soln',1);
%ExyzPlot;

SOLNSstable = readE('CylinderModel_6per_stable-.soln',6);
SOLNSgridelements = readE('CylinderModel_6per_gridelements.soln',6);
diff = SOLNSstable.E{1,6}-SOLNSgridelements.E{1,6};

%% compare solutions
SOLNS = readE('CylinderModel_1per_matrixfree.soln',1);
SOLNSSP = readE('CylinderModel_1per_SP.soln',1);
SOLNSSP2 = readE('CylinderModel_1per_SP2.soln',1);
SOLNSMR = readE('CylinderModel_1per_MR.soln',1);
SOLNSMR0 = readE('CylinderModel_1per_MR_no_coarseness.soln',1);
diffSP = SOLNSSP.E{1,1}-SOLNS.E{1,1};
diffSP2 = SOLNSSP2.E{1,1}-SOLNS.E{1,1};
diffSPSP2 = SOLNSSP2.E{1,1}-SOLNSSP.E{1,1};
max(max(max(abs(diffSP.z))))
max(max(max(abs(diffSP2.z))))
max(max(max(abs(diffSPSP2.z))))
plotEMsoln(SOLNS)
plotEMsoln(SOLNSSP)
plotEMsoln(SOLNSSP2)
plotEMsoln(SOLNSMR)
