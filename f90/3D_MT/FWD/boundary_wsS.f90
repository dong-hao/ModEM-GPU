...
!     start changes for spherical case
    Call zero(E0)

    ! find  radius of surface layer (average radius??)
    r = EARTH_R * KM2M 
    do iSlice = 1,nSlice
       ...
       ! Definitions: imode = 1 Ey polarization and imode = 2 Ex polarization
       if(imode .eq. 2) then
          cosTheta = cos(grid3D%xCenter(iSlice)*pi/180.) !
          do ih = 1,nh
             grid2D%Dy(ih) = grid3D%dy(ih)*pi/180.*r*cosTheta !
          enddo
       else
          !  just assume imode.eq.1
          do ih = 1,nh
             grid2D%Dy(ih) = grid3D%dx(ih)*pi/180.*r !
          enddo
       endif
       ...
       ! now set conductivity for each slice ... differently for
       ! x/y modes
       ! Definitions: imode = 1 Ey polarization and imode = 2 Ex polarization
       if(imode .eq. 2) then
          do ih = 1,nh
